...
function SpelerprofielenDashboard({
  ...
  const developmentRows=perMatch.map((x,i)=>{
    ...
    return {...x,goals60,attempts60,rollingGoals60:rollingMinutes>0?rollingGoals/rollingMinutes*60:0};
  });
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-2xl border bg-white p-5">
      ...
      {previous5.length>=2 && recent5.length>=2 ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{formMetrics.map(m=><div key={m.label} className="rounded-xl border bg-slate-50 p-3"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">{m.label}</div><div className="mt-1 flex items-baseline justify-between gap-2"><div className="text-xl font-black">{m.current.toFixed(1)}{m.suffix}</div><div className={`font-black ${m.cls}`}>{m.arrow} {Math.abs(m.delta).toFixed(1)}{m.suffix}</div></div><div className="mt-1 text-xs text-slate-500">Vorige 5: {m.previous.toFixed(1)}{m.suffix}</div></div>)}</div> : <div className="mt-4 rounded-xl border border-dashed bg-slate-50 p-4 text-sm text-slate-500">Voor een betrouwbare vormvergelijking zijn minimaal twee recente én twee eerdere wedstrijden nodig.</div>}
    </div>
