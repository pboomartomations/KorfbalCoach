...
function MatchReport({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-bold">Scoreverloop</h3><p className="text-xs text-slate-500">Doelpunten in chronologische volgorde.</p></div><div className="text-xs text-slate-400">{progression.length} scoremomenten</div></div><div className="mt-4 flex min-h-16 items-center gap-1.5 overflow-x-auto pb-1">{progression.length?progression.map((e,i)=><div key={e.id+i} className="min-w-[54px] text-center"><div className={`mx-auto flex h-8 w-10 items-center justify-center rounded-lg text-xs font-extrabold text-white ${e.isOpp?'bg-red-500':'bg-green-500'}`}>{e.home}-{e.away}</div><div className="mt-1 text-[9px] text-slate-400">{Math.floor(e.tijdSeconden/60)}'</div></div>):<div className="text-sm text-slate-400">Geen doelpunten geregistreerd.</div>}</div></div>
        <div className="rounded-2xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-bold">Opvallende spelers</h3><p className="text-xs text-slate-500">Spelers die in deze wedstrijd het meest opvallen.</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{playerRows.length?playerRows.map((x,i)=><div key={x.p.id} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-100 font-extrabold text-blue-700">{i+1}</div><div className="min-w-0 flex-1"><div className="truncate font-bold">{x.p.naam}</div><div className="text-xs text-slate-500">{x.goals} goals · {x.rebounds} rebounds · {x.defense} verdedigend</div></div></div>):<div className="text-sm text-slate-400 sm:col-span-2">Nog onvoldoende individuele acties geregistreerd.</div>}</div></div>
      </div>
...
function MatchPreparationDashboard({
  ...
  if(history.length){
    ...
    if(ownAttempts.length) signals.push({tone:ownGoals/ownAttempts>=0.30?"green":"orange",text:`KorbIS scoorde ${((ownGoals/ownAttempts)*100).toFixed(1)}% van de geregistreerde kansen tegen ${opponent}.`});
    if(oppAttempts.length && oppGoals/oppAttempts>=0.30) signals.push({tone:"red",text:`Verdedigend aandachtspunt: tegenstander scoorde ${((oppGoals/oppAttempts)*100).toFixed(1)}% van de kansen.`});
  ...
  return <div className="space-y-5">
    <div className="rounded-3xl border border-blue-100 bg-gradient-to-r from-blue-50 via-white to-cyan-50 p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-gray-500">Historie, patronen en bruikbare aandachtspunten voor de komende wedstrijd.</p>
    </div>
    <div className="rounded-2xl border bg-white p-5">
    ...
    {!opponent ? <div className="rounded-2xl border bg-white p-6 text-sm text-slate-500">Vul bij Wedstrijdinstellingen eerst een tegenstander in. Daarna bouwt KorbIQ deze voorbereiding automatisch op.</div> : !history.length ? <div className="rounded-2xl border bg-white p-6"><h3 className="text-lg font-bold">Eerste ontmoeting in de database</h3><p className="mt-1 text-sm text-slate-500">Er is nog geen historische vergelijking met {opponent}. De huidige vakindeling en wedstrijdregistratie vormen na afloop automatisch de basis voor een volgende voorbeschouwing.</p></div> : <>
...
function SpelerprofielenDashboard({
  ...
  const perMatch = selectedMatches.map((m:any) => {
    ...
    return {id, date:formatImportedDate(m.datum), opponent:String(m.tegenstander ?? m.wedstrijd_naam ?? "-"), minutes:minutesByMatch.get(id) ?? 0, attempts:at.length, goals:g, score:at.length?g/at.length*100:0, quality:at.length?(g+k)/at.length*100:0};
  }).filter(x=>x.minutes>0 || x.attempts>0).sort((a,b)=>a.date.localeCompare(b.date));
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      ...
      <div className="rounded-2xl border bg-white p-5"><h3 className="text-lg font-bold">Bijdrage</h3><div className="mt-4 grid grid-cols-3 gap-3 text-center"><div className="rounded-xl bg-orange-50 p-3"><div className="text-2xl font-black text-orange-700">{rebounds}</div><div className="text-xs font-bold text-orange-800">Rebounds</div></div><div className="rounded-xl bg-blue-50 p-3"><div className="text-2xl font-black text-blue-700">{defense}</div><div className="text-xs font-bold text-blue-800">Verdedigend</div></div><div className="rounded-xl bg-red-50 p-3"><div className="text-2xl font-black text-red-700">{turnovers}</div><div className="text-xs font-bold text-red-800">Balverlies</div></div></div></div>
    </div>
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl border bg-white p-5"><h3 className="text-lg font-bold">Ontwikkeling per wedstrijd</h3><p className="text-sm text-slate-500">Laat zien of prestaties stabiel zijn of veranderen.</p><div className="mt-4 overflow-auto"><table className="w-full min-w-[520px] text-sm"><thead><tr className="text-slate-500"><th className="py-2 text-left">Datum</th><th className="text-left">Tegenstander</th><th className="text-right">Min.</th><th className="text-right">Goals</th><th className="text-right">Kansen</th><th className="text-right">Raak</th></tr></thead><tbody>{perMatch.slice(-10).map(x=><tr key={x.id} className="border-t"><td className="py-2">{x.date}</td><td className="font-semibold">{x.opponent}</td><td className="text-right">{Math.round(x.minutes)}</td><td className="text-right font-bold">{x.goals}</td><td className="text-right">{x.attempts}</td><td className="text-right">{x.attempts?`${x.score.toFixed(1)}%`:'—'}</td></tr>)}</tbody></table></div></div>
